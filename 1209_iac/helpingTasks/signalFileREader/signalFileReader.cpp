...
std::vector<int>* signalFileReader::data(qint64 start, qint64 length, int gidrophoneNumber){
	...
	do{
		...
		int shift = shiftSequently(gidrophoneNumber);
		int channel = channelSequently(gidrophoneNumber);
		...
		while(done < length){
			...
			f->seek(currentTripleInFile + TRIPLE_SIZE);
			currentTripleInFile += TRIPLE_SIZE;

			buffer >> DIFFERENCE_BETWEEN_INT_AND_ADC_IN_BITS;
			res->push_back(buffer);

			// std::cout << measurmentsLeftInBlock << "--" << std::endl;
			...
			if(measurmentsLeftInBlock <= 0){
				...
				/*std::cout << "measurmentsLeftInBlock == 0 ==> skipping to next block" << std::endl;*/
				...
				/*std::cout << "f->seek(" << f->pos() << " + " << OUTER_HEADER_SIZE << ")" << std::endl;*/
...
dualVector<int>* signalFileReader::data(qint64 start, qint64 length){
	...
	std::cout << "data(" << start << ", " << length << ")" << std::endl;
	...
	do{
		...
		while(done < length){
			...
			// std::cout << "f->read(" << rawData << ", " << TRIPLE_SIZE << ")" << std::endl;
			...
			for(int i = 0; i < GIDROPHONES_AMOUNT; i++){
				...
				// std::cout << "memcpy(" << channel << ", " << shift << ", " << ADC_DIMENTION_BYTES << ")" << std::endl;
