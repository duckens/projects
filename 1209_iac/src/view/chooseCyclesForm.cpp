...
chooseCyclesForm::~chooseCyclesForm(){
	log.close();
}
void chooseCyclesForm::reCreateZoomer(){
	// zoomer
...
void chooseCyclesForm::reCreateZoomer2nd(){
	// zoomer
	FFTSignalZoomer = new ScrollZoomer( /*QwtPlot::xBottom, QwtPlot::yLeft, */
		qwtPlot_FFTCycle->canvas());
	FFTSignalZoomer->setRubberBand(QwtPicker::RectRubberBand);
	FFTSignalZoomer->setRubberBandPen(QColor(Qt::green));
	FFTSignalZoomer->setTrackerMode(QwtPicker::ActiveOnly);
	FFTSignalZoomer->setTrackerPen(QColor(Qt::white));

    // panner
	FFTSignalPanner = new QwtPlotPanner(qwtPlot_FFTCycle->canvas());
	FFTSignalPanner->setMouseButton(Qt::MidButton);

	// picker
	FFTSignalPicker = new QwtPlotPicker(QwtPlot::xBottom, QwtPlot::yLeft,
		QwtPlotPicker::CrossRubberBand, QwtPicker::AlwaysOn,
		qwtPlot_FFTCycle->canvas());
	FFTSignalPicker->setStateMachine(new QwtPickerDragPointMachine());
	FFTSignalPicker->setRubberBandPen(QColor(Qt::green));
	FFTSignalPicker->setRubberBand(QwtPicker::CrossRubberBand);
	FFTSignalPicker->setTrackerPen(QColor(Qt::white));

	enableZoomMode2nd(false);
}
void chooseCyclesForm::showPeakSignal1(int gidrophoneNumber){
...
void chooseCyclesForm::showPeakSignal2(int gidrophoneNumber){
...
/*
void chooseCyclesForm::askMainModelToEvaluate(){
...
void chooseCyclesForm::emptyFFTPlot(){
	if(FFTSignal != NULL){
...
void chooseCyclesForm::enableZoomMode(bool on){
...
void chooseCyclesForm::enableZoomMode2nd(bool on){
    FFTSignalPanner->setEnabled(on);
    FFTSignalZoomer->setEnabled(on);
    FFTSignalPicker->setEnabled(!on);
}
void chooseCyclesForm::showCurrentMarker(const QPoint & pos){
	...
	if(!peakSignalZoomer->isEnabled()){
		...
		//QString label = tr("ГЫГЫГЫ cycle: %1").arg(cycleNumber);
		QString label;
...
void chooseCyclesForm::showFFTSignal1(int gidrophoneNumber){

	const bool doReplot = qwtPlot_FFTCycle->autoReplot();

	int DFTDimention = FFTSignal->sizey();
...
void chooseCyclesForm::showFFTSignal2(int gidrophoneNumber){
...
void chooseCyclesForm::proceedCalibrate(){
	std::vector<int>* arrayOfMarkers = listOfMarkers->giveCycleNumbers();
	mainModel->takeCycles(arrayOfMarkers);
	mainModel->calibrate();
}
